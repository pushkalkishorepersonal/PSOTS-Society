...
export async function getModerationConfig(kv, chatId) {
  const key = `_moderation_config_${chatId}`;
  const stored = await kv.get(key);
...
export async function saveModerationConfig(kv, chatId, config) {
  const key = `_moderation_config_${chatId}`;
  await kv.put(key, JSON.stringify(config));
}

export async function storeMessageContext(kv, chatId, message) {
  const key = `_msg_context_${chatId}`;
  const stored = await kv.get(key);
  ...
  await kv.put(key, JSON.stringify(messages), { expirationTtl: 3600 });
}

export async function getMessageContext(kv, chatId, count = 5) {
  const key = `_msg_context_${chatId}`;
  const stored = await kv.get(key);
  if (!stored) return [];
  return JSON.parse(stored).slice(-count);
}
